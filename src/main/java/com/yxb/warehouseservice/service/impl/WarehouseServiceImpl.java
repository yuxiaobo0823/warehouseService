...
public class WarehouseServiceImpl implements IWarehouseService {
    ...
    public Boolean tryWarehouse(String productId, Integer amount) {
        ...
        int existTry = warehouseDao.isExistTry(transId);
        //幂等校验
        if (existTry > 0) {
            log.info("******** tryWarehouse() 已经执行try，无需重复执行..." + transId);
            return null;
        }
        int existConfirm = warehouseDao.isExistConfirm(transId);
        int existCancel = warehouseDao.isExistCancel(transId);
        //悬挂校验
    ...
    public Boolean confirmWarehouse(String productId, Integer amount) {
        ...
        //记录confirm日志
        warehouseDao.addConfirm(transId);
        return null;
    }
    ...
    public Boolean cancelWarehouse(String productId, Integer amount) {
        ...
        //记录回滚日志
        warehouseDao.addCancel(transId);
        return null;
    }
}
